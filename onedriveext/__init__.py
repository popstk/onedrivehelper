...
class OneDriveClient(object):
    ...
    def upload(self, src, dest="/", chunksize=1024*1024*10, persist=None, cancel_func=None, chunk_func=None):
        ...
        if session is None:
            session = upload.create_session(
                self.client.api_base_url, token, src, dest)
        ...
        while offset < total:
            ...
            if chunk_func(respond, this_range):
                offset += chunksize
        else:
            persist.Del(src)
            return True
    ...
    def load_session(path=""):
        ...
        return OneDriveClient(od_session.load_session(None,  os.path.expanduser(path)))
